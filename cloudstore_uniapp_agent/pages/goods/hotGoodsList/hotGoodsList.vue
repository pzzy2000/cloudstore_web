      ...
      热门活动商品</nav-bar>
		...
		<view class="hot-list">
			<view v-for="(item, index) in hotList" :key="index" class="goods-item" @click="navToDetailPage(item)">
				<view class="image-wrapper"><image :src="item.goodsPhotos[0].url" mode="aspectFill"></image></view>
				<view class="goods-detail">
					<text class="title clamp">{{item.goodsBean.goodsName}}</text>
					<text class="title clamp subhead ">{{item.goodsBean.goodsSubtitle}}</text>
					<view class="price-box">
						<view class="price">
							<text class="priceSale">{{item.goodsBean.salePrice}}</text>
							/
							<text class="pricemart">{{item.goodsBean.martPrice}}</text>
						</view>
