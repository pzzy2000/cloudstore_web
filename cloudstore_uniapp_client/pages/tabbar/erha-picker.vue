<template>
	<view class="level-container">
		<h1 class="title">选择收货地址 <A @click="exit">取消</A> </h1>
		<ul class="name-list">
			<li :class="showIndex==0?'select':''" @click="anewSelect(0)">{{province.name}}</li>
			<li :class="showIndex==1?'select':''" @click="anewSelect(1)" v-if="showIndex>=1">{{city.name}}</li>
			<li :class="showIndex==2?'select':''" @click="anewSelect(2)" v-if="showIndex>=2">{{area.name}}</li>
			<!--
			<li :class="showIndex==3?'select':''" @click="anewSelect(3)" v-if="showIndex>=3">{{street}}</li>
			-->
			
		</ul>
		<ul v-if="showIndex==0" class="content" :style="'height:'+ heightCot + 'upx'">
			<li @click="selectPro(index,item)" v-for="(item,index) in provinceData" :key="item.id">{{item.name}}</li>
		</ul>

		<ul v-if="showIndex==1" class="content" :style="'height:'+ heightCot + 'upx'">
			<li @click="selectCity(index,item)" v-for="(item,index) in cityData" :key="item.id">{{item.name}}</li>
		</ul>

		<ul v-if="showIndex==2" class="content" :style="'height:'+ heightCot + 'upx'">
			<li @click="selectaArea(index,item)" v-for="(item,index) in areaData" :key="item.id">{{item.name}}</li>
		</ul>
		<!-- 
		<ul v-if="showIndex==3" class="content" :style="'height:'+ heightCot + 'upx'">
			<li @click="selectStreet(index,item)" v-for="(item,index) in streetsData" :key="item">{{item}}</li>
		</ul>
		-->

		
	</view>
</template>

<script>
	// import provinceData from './city-data/province.js';
	// import getCity from './city-data/city.js';
	// import getArea from './city-data/area.js';
	// import getStreets from './city-data/streets.js';
	import Api from '@/common/api';
	export default {
		data() {
			return {
				province: {
					name:"省/直辖市",
					id:-1
				},
				city:  {
					name:"市",
					id:-1
				},
				area:  {
					name:"区/县",
					id:-1
				},
				street: "请选择",
				cityDataAll: '', // 市的所有數據
				getAreaAll: '', // 区的所有数据
				getStreetsAll: '', // 街道的所有数据
				presentIndex: [],
				proIndex: 0,
				cityIndex: 0,
				areaIndex: 0,

				provinceData: '', // 当前展示的省数据
				cityData: '', // 当前展示的市数据
				areaData: '', //当前展示的区数据
				streetsData: '', //当前展示的区数据
				showIndex: 0,
				heightCot: 0,
				bb: ''
			}
		},
		created() {
			uni.getSystemInfo({
				success: res => {
					console.log(res.safeArea.height)
					this.heightCot = (res.safeArea.height * 2) /3
				}
			})
		},
		mounted() {
			
			let serp = this.setProvinceData;
			this.selectareas(0,function(data){
				serp(data);
			});
			
			// console.log(provinceData)
			// this.provinceData = provinceData;
			// this.cityDataAll = getCity;
			// this.getAreaAll = getArea;
			// this.getStreetsAll = getStreets
		},
		
		onLoad() {
		
		},
		methods: {
			
			setProvinceData(data){
				this.provinceData = data;
			},
			
			async selectareas(parentId,cackcall){
				let params ={pageNum:1,pageSize:50,parentId:parentId,dirctType:'areas'};
				let data = await Api.apiCall('post', Api.areas.province, params, true, true);
				if(data){
					  let data_ =  data.result.records;
					  cackcall(data_);
				}
			},
			
			anewSelect(num) {
				switch (num) {
					case 0:
						this.showIndex = 0;
						this.areaData = [];
						this.streetsData = [];
						this.city={
							name:"市",
							id:-1
						};
						this.area= {
							name:"区/县",
							id:-1
						};
						this.street=  {
							name:"社区",
							id:-1
						};
						break;
					case 1:
						this.showIndex = 1;
						this.areaData = [];
						this.streetsData = [];
						this.area= {
							name:"区/县",
							id:-1
						};
						this.street=  {
							name:"社区",
							id:-1
						};
						break;
					case 2:
						this.showIndex = 2;
						this.streetsData = [];
						this.street={
							name:"社区",
							id:-1
						};
						break;
					case 3:
						console.log(11111);
						break;
				}
			},
			
			setCityData(data,item,index){
				this.cityData = data;
				this.proIndex = index; // 当前省的下标
				this.province = item;
				this.showIndex = 1;
			},
			selectPro(index, item) {
				console.log(index)
				let setCity = this.setCityData;
				this.selectareas(item.id,function(data){
					setCity(data,item,index);
				});
				
				
			},
			
			setAreaData(data,item,index){
				this.areaData = data;
				this.city = item;
				this.cityIndex = index; // 当前市的下标
				this.showIndex = 2;
			},
			selectCity(index, item) {
				let setArea = this.setAreaData;
				this.selectareas(item.id,function(data){
					setArea(data,item,index);
				});
				
			},
			
			setStreetdata(data,item,index){
				this.areaData= data;
				this.area = item;
				this.areaIndex = index; // 当前市的下标
				this.showIndex = 3;
			},
			
			exit(){
				this.$emit('close')
			},
			selectaArea(index, item) {
				
				this.area = item;
				this.areaIndex = index; // 当前市的下标
				this.showIndex = 3;
				
					const {
						province,
						city,
						area
						// street
					} = this;
			this.$emit('conceal',{
						province,
						city,
						area
					})
					// this.showIndex = 4;
				
				// let setStreet = this.setStreetdata;
				// this.selectareas(item.id,function(data){
				// 	setStreet(data,item,index);
				// });
				
				// const {
				// 	proIndex,
				// 	cityIndex
				// } = this;
				// this.area = label;
				// this.showIndex = 3;
				// console.log(index)
				// console.log(this.getStreetsAll[proIndex][cityIndex])
				// console.log(this.getStreetsAll[proIndex][cityIndex][index])
				// this.streetsData = this.getStreetsAll[proIndex][cityIndex][index]
			},
			// selectStreet(index, label) {
			// 	this.street = label;
			// 	const {
			// 		province,
			// 		city,
			// 		area,
			// 		street
			// 	} = this;
			// 	this.$emit('conceal',{
			// 		province,
			// 		city,
			// 		area,
			// 		street
			// 	})
			// 	// this.showIndex = 4;
			// }
		}
	}
</script>

<style lang="less">
	ul,li{
		list-style: none;
	}
	.level-container {
		width: 100%;
		background: #fff;
		position: fixed;
		bottom: 0;

		.select {
			color: red;
			position: relative;

			&::after {
				content: '';
				width: 40upx;
				height: 6upx;
				background: red;
				position: absolute;
				left: 50%;
				bottom: -8upx;
				margin-left: -20upx;
			}
		}

		li {
			font-size: 26upx;
		}

		.content,
		.name-list {
			padding: 0 20upx;
		}

		.title {
			font-size: 32upx;
			margin: 30upx 20upx;
		}

		.name-list {
			display: flex;

			li {
				margin-right: 40upx;
				padding-bottom: 6upx;
			}
		}

		.content {
			margin-top: 20upx;
			height: 600upx;
			overflow-y: auto;

			li {
				border-bottom: 1px solid #f1f1f1;
				padding: 20upx 0;
			}

			li:last-child {
				border-bototm: none !important;
			}
		}
	}
</style>
