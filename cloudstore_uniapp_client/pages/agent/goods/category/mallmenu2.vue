							...
							<view class="goods-detail">
								...
								<view class="detail-price">
									...
									<view class="price-btn">
										<button class="price-share" @click.stop='onShareSave(item)'><text>{{isType()+item.client}}</text></button>
...
<script>
	...
	export default {
		...
		onLoad() {
			...
			this.netWork().getGoodsOneType() //初始化调用获取一级分类列表
		},
		...
		methods: {
			netWork () { //网络层
				return {
					getGoodsOneType: async () => { //初始化加载商品一级分类
						let params = {
							pageNum: 1,
							parentId: 0
						};
						...
						if (list) {
							if (list.result.records.length != 0) {
								...
								this.categoryOneId = this.goodsOneType[0].value //赋值一级分类的id
			...
			// 			this.havStock = ''
			// 			// this.selectGoodsBrandList.length = 0
			// 			// this.selectGoodsBrandListValue = ''
			// 			console.log(this.selectGoodsBrandList)
			// 			console.log(this.selectGoodsBrandListValue)
			// 			// this.goodsList.length = 0 //商品列表清空
			// 		}
			// 	}
			// },
			//以下为事件层
			onGoodsOneType (index, value) {
				this.mallmenuOneTypeCurrent = index
				this.categoryOneId = value
				this.goodsList.length = 0
				this.mallmenuTwoTypeCurrent = 0
