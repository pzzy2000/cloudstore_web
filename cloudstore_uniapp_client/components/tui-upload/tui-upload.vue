					...
					if (token) {
						uni.uploadFile({
							...
							success: function(res) {
								...
								if (resData.code === 0) {
									_this.imgListId[index] = resData.result.id
									_this.imageList[index] = resData.result.url
									_this.$set(_this.statusArr, index, resData.result.url ? "1" : "3")
								} else {
									// 上传失败
