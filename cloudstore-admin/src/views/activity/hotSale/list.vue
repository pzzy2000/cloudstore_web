...
<!--    </el-card>-->
    <el-card class="operate-container" shadow="never">
      ...
      <span>数据列表</span>
      <el-button size="mini" style="float: right" @click="addactivity" v-if="powershowing(power.hotSale_add)">添加活动</el-button>
      <!--
      <el-button
        class="btn-add"
...
<!--&lt;!&ndash;          <template slot-scope="scope">{{scope.row.agentBean.name}}</template>&ndash;&gt;-->
<!--        </el-table-column>-->
...
<!--        </el-table-column>-->
        ...
        <el-table-column label="操作" width="270" align="center">
          <template slot-scope="scope">
            <el-button type="primary" size="mini" @click="associatedGood(scope.row)" v-if="powershowing(power.hotSale_assoGoods)">关联商品</el-button>
            <el-button size="mini" @click="updateAct(scope.row)" v-if="powershowing(power.hotSale_update)">修改活动</el-button>
            <el-button type="danger" size="mini" @click="handeldelGoods(scope.row)" v-if="powershowing(power.hotSale_delete)" :disabled="scope.row.isDelete == 1 ? true : false || scope.row.status == 1 ? true : false">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      addactivity() {
        this.$router.push({
          path: "/sys/activity/hotSaleaddact"
          // query: {rds: "write"}
        })
      },
      associatedGood(row) {
        this.$router.push({
          path: "/sys/activity/hotSaleassoGoods",
      ...
      updateAct(row) {
        this.$router.push({path: "/sys/activity/hotSaleaddact", query: {id: row.id}})
      }
    }
  }
</script>
