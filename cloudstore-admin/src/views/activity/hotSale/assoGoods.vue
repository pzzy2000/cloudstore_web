...
<!--          </el-form-item>-->
    ...
    <el-card class="operate-container" shadow="never">
      ...
      <el-button type="primary" size="mini" @click="associatedGood" style="float: right; margin-right: 20px" v-if="powershowing(power.hotSale_goodslist)">关联商品</el-button>
    </el-card>
...
<!--            <el-button size="mini" @click="readInfo(scope.row)">查看详情</el-button>-->
            <el-button type="primary" size="mini" @click="settingactPrice(scope.row)" v-if="powershowing(power.hotSale_setparams)">设置参数</el-button>
<!--            <el-button size="mini" @click="qrcode(scope.$index, scope.row)" v-if="powershowing(power.outline_qrcode)">生成二维码</el-button>-->
            <el-button type="danger" size="mini" @click="handeldel(scope.row)" v-if="powershowing(power.hotSale_cancelAsso)">取消关联</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      settingactPrice(row) {
        this.$router.push({path: '/sys/activity/hotSalesetParams', query: {id: row.skuId, activityId: this.$route.query.id, activityGoodsId: row.activityGoodsId, offlinePrice: row.offlinePrice}})
      },
      // qrcode(index, row) {
      ...
      associatedGood(row) {
        ...
        this.$router.push({
          path: "/sys/activity/hotSaleactGoodslist",
          query: {activityid: id}
        })
      },
      handeldel(row) {
