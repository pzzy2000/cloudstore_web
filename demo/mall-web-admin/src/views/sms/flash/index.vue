  ...
  <div class="app-container">
      ...
      <div style="margin-top: 15px">
        <el-form :inline="true" :model="listQuery" size="small" label-width="140px">
          <el-form-item label="活动名称：">
            <el-input v-model="listQuery.title" class="input-width" placeholder="活动名称" clearable></el-input>
          </el-form-item>
        </el-form>
      </div>
    </el-card>
    ...
    <div class="table-container">
      <el-table ref="flashTable"
        ...
        <el-table-column label="是否首页显示" width="200" align="center">
          <template slot-scope="scope">
            <el-switch
              @change="handleIsIndexChange(scope.$index, scope.row)"
              :active-value="1"
              :inactive-value="0"
    ...
    <el-dialog
      ...
      <el-form :model="flashPromotion"
        ...
        <el-form-item label="首页显示">
          <el-radio-group v-model="flashPromotion.isIndex">
...
<script>
  import {fetchList, updateStatus,updateIsIndex, deleteFlash, createFlash, updateFlash} from '@/api/flash';
  ...
  export default {
    ...
    filters: {
      formatActiveStatus(row) {
        ...
        let start = new Date(row.startDate).getTime();
        let end = new Date(row.endDate).getTime();
    ...
    methods: {
      ...
      handleIsIndexChange(index, row) {
        this.$confirm('是否要首页显示?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          let data = new URLSearchParams();
          data.append("ids", row.id);
          data.append("isIndex", row.isIndex);
          updateIsIndex(data).then(response => {
            this.$message({
              message: '修改成功',
              type: 'success',
              duration: 1000
