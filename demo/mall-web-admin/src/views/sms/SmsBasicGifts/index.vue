<template> 
  <div class="app-container">
    ...
    <div class="table-container">
      <el-table ref="SmsBasicGiftsTable"
                :data="list"
                style="width: 100%;"
        ...
        <el-table-column label="优惠类型" width="120" align="center">
          <template slot-scope="scope">{{scope.row.smallType | formatCouponType(scope.row)}}</template>
        </el-table-column>
  ...
  <el-table-column label="活动规则" width="240" align="center">
          <template slot-scope="scope">{{scope.row.rules|filterRule(scope.row)}}</template>
        </el-table-column>
...
<script>
  import {fetchList,updateStatus,deleteSmsBasicGifts} from '@/api/sms/smsBasicGifts';
  ...
  export default {
    ...
    methods: {
      ...
      handleUpdate(index,row){
        this.$router.push({path: '/sms/updateSmsBasicGifts', query: {id: row.id}})
      },
      getList() {
